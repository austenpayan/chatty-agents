export default function makePrompt(
    position: string,
    previousResponses: string[] = [],
    opponentPosition: string,
    lastOpponentsResponse?: string
) {
    let str = "";

    str +=
        "You are in a spirited but playful banter debate with another person. ";
    ...
    if (previousResponses.length) {
        str +=
            "You have already responded with these arguments, do not use the same argument: ";
